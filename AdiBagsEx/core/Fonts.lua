...
-- Initialize font defaults when addon is ready
local function InitializeFontDefaults()
	if addon.InitializeFontDefaults then
		addon:InitializeFontDefaults()
	end
end

-- Delay initialization until addon is fully loaded
if addon.db then
	InitializeFontDefaults()
else
	-- Set up a timer to check when addon is ready
	local checkTimer
