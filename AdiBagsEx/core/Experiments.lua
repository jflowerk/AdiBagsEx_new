...
function Experiments:OnInitialize()
  -- Experiments will be initialized by the main addon after database is ready
end
...
function Experiments:Enabled(name)
  -- Check if experiments database exists and is initialized
  if not addon.db or not addon.db.profile or not addon.db.profile.experiments then
    return false
  end
  
  -- Check if the specific experiment exists
  if addon.db.profile.experiments[name] == nil then
    -- Experiment doesn't exist yet, return false instead of erroring
    return false
  end
