...
-- Define bag keys based on WoW version
local bagKeys = {"backpack", "bank"}
local interfaceVersion = select(4, GetBuildInfo())
if interfaceVersion < 110200 then
	-- Reagent bank was removed in WoW 11.2.0 for all versions
	table.insert(bagKeys, "reagentBank")
end
if addon.isRetail and interfaceVersion >= 110000 then
	-- Warbound bank was added in WoW 11.0 (The War Within) - retail only
	table.insert(bagKeys, "warbandbank")  -- lowercase to match theme lookup
end
function addon:OnInitialize()
	-- Migrate settings from AdiBagsT to AdiBagsEx
	if _G.AdiBagsTDB and not _G.AdiBagsExDB then
		_G.AdiBagsExDB = _G.AdiBagsTDB
		print("|cff00ff00AdiBagsEx:|r Migrated settings from AdiBagsT")
	end
	
	-- Create the default font settings for each bag type.
	for _, name in ipairs(bagKeys) do
		-- Ensure the theme entry exists for this bag type
		if not addon.DEFAULT_SETTINGS.profile.theme[name] then
			addon.DEFAULT_SETTINGS.profile.theme[name] = {
				background = "Blizzard Dialog Background",
				border = "Blizzard Tooltip",
				borderWidth = 16,
				insets = 3,
				color = name == "warbandbank" and { 0.5, 0, 0.5, 1 } or { 0, 0, 0, 1 },
		...
		addon.DEFAULT_SETTINGS.profile.theme[name].bagFont = bfd
		addon.DEFAULT_SETTINGS.profile.theme[name].sectionFont = self:GetFontDefaults(GameFontNormalLeft)
	end
	...
	-- Initialize experiments now that the database is ready
	local Experiments = self:GetModule("Experiments")
	if Experiments and Experiments.CreateAllExperiments then
		Experiments:CreateAllExperiments()
	end
	...
	-- Persistant handlers
	self.RegisterBucketMessage(addonName, 'AdiBagsEx_ConfigChanged', 0.2, function(...) addon:ConfigChanged(...) end)
	self.RegisterEvent(addonName, 'PLAYER_ENTERING_WORLD', function() if self.db.profile.enabled then self:Enable() end end)

	self:RegisterChatCommand("adibagsex", function(cmd)
		if cmd and cmd:trim() ~= "" then
			local args = {strsplit(' ', cmd:trim())}
			...
			addon:OpenOptions(unpack(filteredArgs))
		else
			addon:OpenOptions()
		end
	end, true)
...
--------------------------------------------------------------------------------
-- Event Handlers (must be defined before OnEnable)
--------------------------------------------------------------------------------

local updatedBags = {}
local updatedBank = { [BANK_CONTAINER] = true }
local updatedReagentBank = {}
local updatedWarboundBank = {}
-- Check if reagent bank is available in this version
local interfaceVersion = select(4, GetBuildInfo())
if addon.isRetail and interfaceVersion < 110200 then
	-- Reagent bank was removed in WoW 11.2.0
	updatedReagentBank = { [REAGENTBANK_CONTAINER] = true }
end
-- Check if warbound bank is available in this version
if addon.isRetail and interfaceVersion >= 110000 then
	-- Warbound bank was added in WoW 11.0 (The War Within)
	updatedWarboundBank = {
		[WARBOUND_BANK_CONTAINER_1] = true,
		[WARBOUND_BANK_CONTAINER_2] = true,
		[WARBOUND_BANK_CONTAINER_3] = true,
		[WARBOUND_BANK_CONTAINER_4] = true,
		[WARBOUND_BANK_CONTAINER_5] = true,
	}
end
...
function addon:BAG_UPDATE(event, bag)
	...
	-- Check if reagent bank is available and handle accordingly
	local interfaceVersion = select(4, GetBuildInfo())
	if bag == REAGENTBANK_CONTAINER then
		if interfaceVersion >= 110200 then
			-- Reagent bank was removed in 11.2.0 for all versions, ignore these updates
			return
		else
			-- For older versions, handle reagent bank updates
			self:SendMessage('AdiBags_BagUpdated', updatedReagentBank)
			return
		end
	end
	
	-- Check if warbound bank and handle accordingly
	if addon.isRetail and interfaceVersion >= 110000 then
		if bag == WARBOUND_BANK_CONTAINER_1 or bag == WARBOUND_BANK_CONTAINER_2 or 
		   bag == WARBOUND_BANK_CONTAINER_3 or bag == WARBOUND_BANK_CONTAINER_4 or 
		   bag == WARBOUND_BANK_CONTAINER_5 then
			self:SendMessage('AdiBags_BagUpdated', updatedWarboundBank)
			return
		end
	end
...
function addon:OnEnable()
	...
	-- PLAYERREAGENTBANKSLOTS_CHANGED was removed in WoW 11.2.0 for all versions
	-- Reagent bank updates are now handled by BAG_UPDATE event
	local interfaceVersion = select(4, GetBuildInfo())
	if interfaceVersion < 110200 then
	...
	if self.db and self.db.profile then
		...
		addon:Debug('Database not ready in OnEnable, skipping SetSortingOrder')
		return
	end
...
function addon:OnDisable()
	self.anchor:Hide()
	self:CloseAllBags()
	-- Unregister PLAYERREAGENTBANKSLOTS_CHANGED only if it was registered
	local interfaceVersion = select(4, GetBuildInfo())
	if interfaceVersion < 110200 then
...
local prevSkinPreset = {
  ...
  WarboundBankColor = { 0.5, 0, 0.5, 1 }, -- Purple for Warbound Bank
}

function addon:UpgradeProfile()
	-- Ensure new theme entries exist for existing profiles
	for _, name in ipairs(bagKeys) do
		if not self.db.profile.theme[name] then
			self.db.profile.theme[name] = {
				...
				insets = 3,
				color = name == "warbandbank" and { 0.5, 0, 0.5, 1 } or { 0, 0, 0, 1 },
				bagFont = self:GetFontDefaults(GameFontHighlightLarge),
				sectionFont = self:GetFontDefaults(GameFontNormalLeft),
			}
		end
	end
	...
	if skin then
		for _, key in ipairs(bagKeys) do
			...
			elseif key == "warbandbank" and skin.WarboundBankColor then
				for i, v in ipairs(prevSkinPreset.WarboundBankColor) do
					v = skin.WarboundBankColor[i] or v
					addon.db.profile.theme[key].color[i] = v
				end
			end
		end
...
do
	local configAddonName = "AdiBagsEx_Config"
	local why = '???'
	local function CouldNotLoad()
		print("|cffff0000AdiBagsEx:", format(ADDON_LOAD_FAILED, configAddonName, why), "|r")
	end
...
function addon:ConfigChanged(vars)
	...
	elseif vars.filter then
		return self:SendMessage('AdiBagsEx_FiltersChanged')
	else
		for name in pairs(vars) do
			...
			elseif strmatch(name, 'columnWidth') then
				return self:SendMessage('AdiBagsEx_LayoutChanged')
			end
		end
	end
...
function addon:SetGlobalLock(locked)
	...
	if locked ~= self.globalLock then
		...
		if not locked then
			self:SendMessage('AdiBagsEx_LayoutChanged')
		end
		return true
	end
end
