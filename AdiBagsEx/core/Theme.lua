...
function addon:SetTheme(name)
  ...
  local themeKeys = {'backpack', 'bank'}
  -- Only include reagentBank if it exists in this version
  if self.db.profile.theme.reagentBank then
    table.insert(themeKeys, 'reagentBank')
  end
...
function addon:NewTheme(name)
  ...
  -- Only copy reagentBank if it exists in this version
  if addon.db.profile.theme.reagentBank then
    theme.reagentBank = _G.CopyTable(addon.db.profile.theme.reagentBank)
  end
