...
function addon:ToggleBag(id)
	local ourBag = hookedBags[id]
	if ourBag then
		-- Check if this is a bank module and we're not at a bank
		if ourBag.isBank and addon:GetInteractingWindow() ~= "BANKFRAME" then
			-- Don't toggle bank containers when not at bank
			return
		end
