...
function addon:SetupDefaultFilters()
	...
	self:SetCategoryOrders{
		...
		[TRADE_GOODS] = 20,
		[EQUIPMENT] = 10,
		[L['Bind on Pickup'] or 'Bind on Pickup'] = 5,
		[L['Bind on Equip'] or 'Bind on Equip'] = 0,
		[CONSUMMABLE] = -10,
		[MISCELLANEOUS] = -20,
		[AMMUNITION] = -30,
		[JUNK] = -40,
	...
	if addon.isRetail or addon.isWrath or addon.isCata then
		do
			...
			function setFilter:EQUIPMENT_SETS_CHANGED(event)
				self:UpdateNames()
				self:SendMessage('AdiBagsEx_FiltersChanged', true)
			end
				...
				return {
					...
					mergedSets = {
						...
						set = function(info, name, value)
							...
							self:SendMessage('AdiBagsEx_FiltersChanged')
						end,
	...
	-- [50] Bound Items Filter (Modernized)
	do
		local boundFilter = addon:RegisterFilter("BoundItems", 50, "ABEvent-1.0")
		boundFilter.uiName = L['Bound Items'] or 'Bound Items'
		boundFilter.uiDesc = L['Put bound gear items (BOE/BOP/Warbound) in their own sections.'] or 'Put bound gear items (BOE/BOP/Warbound) in their own sections.'

		-- Modern binding detection
		local modernBinding = addon:GetModule('ModernBinding', true)

		-- Fallback constants for compatibility
		local LE_ITEM_BIND_ON_ACQUIRE = _G.LE_ITEM_BIND_ON_ACQUIRE or (Enum.ItemBindType and Enum.ItemBindType.LE_ITEM_BIND_ON_ACQUIRE) or 1
		local LE_ITEM_BIND_ON_EQUIP = _G.LE_ITEM_BIND_ON_EQUIP or (Enum.ItemBindType and Enum.ItemBindType.LE_ITEM_BIND_ON_EQUIP) or 2
		local LE_ITEM_BIND_TO_BNETACCOUNT = _G.LE_ITEM_BIND_TO_BNETACCOUNT or (Enum.ItemBindType and Enum.ItemBindType.LE_ITEM_BIND_TO_BNETACCOUNT) or 5
		local LE_ITEM_BIND_TO_ACCOUNT = _G.LE_ITEM_BIND_TO_ACCOUNT or (Enum.ItemBindType and Enum.ItemBindType.LE_ITEM_BIND_TO_ACCOUNT) or 6

		function boundFilter:OnInitialize()
			self.db = addon.db:RegisterNamespace('BoundItems', {
				profile = {
					separateBOE = true,
					separateBOP = true,
					separateWarbound = true,
					separateWUE = true,    -- Warbound until Equip
					separateSoulbound = true,
					gearOnly = true,
					useModernDetection = true,  -- Enable modern binding detection
				}
			})
		end

		function boundFilter:Filter(slotData)
			if not slotData.bindType then return end
			
			-- Check if we should only apply to gear items
			if self.db.profile.gearOnly then
				local equipSlot = slotData.equipSlot
				if not equipSlot or equipSlot == "" or equipSlot == "INVTYPE_NON_EQUIP_IGNORE" then
					return -- Not equipment, skip this filter
				end
			end

			local bindingInfo
			
			-- Use modern binding detection if available and enabled
			if self.db.profile.useModernDetection and modernBinding then
				bindingInfo = modernBinding:GetItemBindingFromBagSlot(slotData.bag, slotData.slot, slotData.bindType)

				-- Use modern binding scope detection
				if bindingInfo.binding == Enum.BindingScope.BOE and self.db.profile.separateBOE then
					return L['Bind on Equip'] or 'Bind on Equip'
				elseif bindingInfo.binding == Enum.BindingScope.BOUND and self.db.profile.separateBOP then
					-- Generic bound (BOP)
					return L['Bind on Pickup'] or 'Bind on Pickup'
				elseif bindingInfo.binding == Enum.BindingScope.ACCOUNT and self.db.profile.separateWarbound then
					return L['Warbound'] or 'Warbound'
				elseif bindingInfo.binding == Enum.BindingScope.WUE and self.db.profile.separateWUE then
					return L['Warbound'] or 'Warbound'  -- WUE shows as Warbound for now
				elseif bindingInfo.binding == Enum.BindingScope.SOULBOUND and self.db.profile.separateSoulbound then
					return L['Soulbound'] or 'Soulbound'
				end
			else
				-- Fallback to legacy detection
				if slotData.bindType == LE_ITEM_BIND_ON_EQUIP and self.db.profile.separateBOE then
					return L['Bind on Equip'] or 'Bind on Equip'
				elseif slotData.bindType == LE_ITEM_BIND_ON_ACQUIRE and self.db.profile.separateBOP then
					return L['Bind on Pickup'] or 'Bind on Pickup'
				elseif (slotData.bindType == LE_ITEM_BIND_TO_BNETACCOUNT or slotData.bindType == LE_ITEM_BIND_TO_ACCOUNT) and self.db.profile.separateWarbound then
					return L['Warbound'] or 'Warbound'
				end
			end
		end

		function boundFilter:GetOptions()
			return {
				gearOnly = {
					name = L['Equipment only'] or 'Equipment only',
					desc = L['Only apply binding filters to equipment items (weapons, armor, accessories).'] or 'Only apply binding filters to equipment items (weapons, armor, accessories).',
					type = 'toggle',
					order = 5,
				},
				useModernDetection = {
					name = L['Use Modern Detection'] or 'Use Modern Detection',
					desc = L['Use advanced binding detection for more accurate warbound vs soulbound identification.'] or 'Use advanced binding detection for more accurate warbound vs soulbound identification.',
					type = 'toggle',
					order = 8,
				},
				separateBOE = {
					name = L['Separate BOE items'] or 'Separate BOE items',
					desc = L['Put "Bind on Equip" items in their own section.'] or 'Put "Bind on Equip" items in their own section.',
					type = 'toggle',
					order = 10,
				},
				separateBOP = {
					name = L['Separate BOP items'] or 'Separate BOP items',
					desc = L['Put "Bind on Pickup" items in their own section.'] or 'Put "Bind on Pickup" items in their own section.',
					type = 'toggle',
					order = 20,
				},
				separateWarbound = {
					name = L['Separate Warbound items'] or 'Separate Warbound items',
					desc = L['Put "Warbound" items in their own section.'] or 'Put "Warbound" items in their own section.',
					type = 'toggle',
					order = 30,
				},
				separateSoulbound = {
					name = L['Separate Soulbound items'] or 'Separate Soulbound items',
					desc = L['Put "Soulbound" items in their own section (requires modern detection).'] or 'Put "Soulbound" items in their own section (requires modern detection).',
					type = 'toggle',
					order = 40,
					disabled = function() return not self.db.profile.useModernDetection end,
				},
			}, addon:GetOptionHandler(self, true)
		end
