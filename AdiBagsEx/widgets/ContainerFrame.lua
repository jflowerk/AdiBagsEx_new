...
-- Use modernized constants from Constants.lua
local BANK_CONTAINER = addon.BANK_CONTAINER
local REAGENTBANK_CONTAINER = addon.REAGENTBANK_CONTAINER
-- Account Bank containers are now defined in Constants.lua
local WARBOUND_BANK_CONTAINER_1 = addon.ACCOUNT_BANK_CONTAINER_1
local WARBOUND_BANK_CONTAINER_2 = addon.ACCOUNT_BANK_CONTAINER_2
local WARBOUND_BANK_CONTAINER_3 = addon.ACCOUNT_BANK_CONTAINER_3
local WARBOUND_BANK_CONTAINER_4 = addon.ACCOUNT_BANK_CONTAINER_4
local WARBOUND_BANK_CONTAINER_5 = addon.ACCOUNT_BANK_CONTAINER_5
local ceil = _G.ceil
...
-- Helper function for WoW 11.2.0 compatibility
-- Reagent Bank was removed in WoW 11.2.0
local function IsReagentBankAvailable()
	local interfaceVersion = select(4, GetBuildInfo())
	if interfaceVersion >= 110200 then
		return false -- Reagent Bank removed in 11.2.0
	end
	-- For older versions, check if unlocked
	if IsReagentBankUnlocked then
		return IsReagentBankUnlocked()
	elseif C_Bank and C_Bank.IsReagentBankUnlocked then
		return C_Bank.IsReagentBankUnlocked()
	else
		return true -- Assume available in older versions
	end
end

-- Helper function to check if Warbound Bank is available
local function IsWarboundBankAvailable()
	local interfaceVersion = select(4, GetBuildInfo())
	if not addon.isRetail or interfaceVersion < 110000 then
		return false -- Warbound Bank added in WoW 11.0
	end
	
	-- Check if the containers are accessible
	-- In 11.2.0+, we use C_Container API if available
	if C_Container and C_Container.GetContainerNumSlots then
		local slots = C_Container.GetContainerNumSlots(WARBOUND_BANK_CONTAINER_1)
		return slots and slots > 0
	elseif GetContainerNumSlots then
		local slots = GetContainerNumSlots(WARBOUND_BANK_CONTAINER_1)
		return slots and slots > 0
	end
...
local function GetReagentBankCostCompat()
	if GetReagentBankCost then
		return GetReagentBankCost()
	elseif C_Bank and C_Bank.GetReagentBankCost then
		return C_Bank.GetReagentBankCost()
	else
		-- Fallback cost if function doesn't exist
		return 10000 -- 1 gold
	end
end
...
function containerProto:OnCreate(name, isBank, bagObject)
	...
	local ids
	-- Use the bag object's specific bag IDs instead of generic lookup
	for bagId in pairs(self.bagObject.bagIds or {}) do
	...
	if addon.isRetail then
		if self.isBank then
			-- Only create reagent tab button if reagent bank is available (removed in 11.2.0)
			local interfaceVersion = select(4, GetBuildInfo())
			if interfaceVersion < 110200 then
				self:CreateReagentTabButton()
			end
			self:CreateDepositButton()
			self:CreateWarbandBankButton()
		end
	...
	RegisterMessage(name, 'AdiBagsEx_FiltersChanged', self.FullUpdate, self)
	RegisterMessage(name, 'AdiBagsEx_LayoutChanged', self.FullUpdate, self)
	RegisterMessage(name, 'AdiBagsEx_ConfigChanged', self.ConfigChanged, self)
	...
	if addon.isRetail then
		...
		if isBank then
			...
			if C_Container then
				...
				-- SortReagentBankBags was removed in WoW 11.2.0
				if interfaceVersion < 110200 and C_Container.SortReagentBankBags then
					hooksecurefunc(C_Container, 'SortReagentBankBags', ForceFullLayout)
				end
			else
				hooksecurefunc('SortBankBags', ForceFullLayout)
				-- SortReagentBankBags was removed in WoW 11.2.0
				if interfaceVersion < 110200 and SortReagentBankBags then
...
function containerProto:CreateDepositButton()
	...
	-- Check if reagent bank is available and unlocked (WoW 11.2.0 compatibility)
	local interfaceVersion = select(4, GetBuildInfo())
	if interfaceVersion >= 110200 then
		-- Reagent Bank was removed in 11.2.0, hide the button
		button:Hide()
		return
	end
	
	if not IsReagentBankAvailable() then
		button:Hide()
...
function containerProto:CreateWarbandBankButton()
	-- Only show this button for retail and if we're at a bank
	if not addon.isRetail or not self.isBank then
		return
	end
	...
	local button = self:CreateModuleButton(
		...
		5,
		function()
			-- Debug: Check if we're at a bank
			if not addon.atBank then
				print("|cffff0000AdiBagsEx:|r You must be at a bank to use this feature.")
				return
			end
			
			-- Toggle warband bank frame visibility
			local warbandBag = addon:GetModule("WarbandBank")
			if warbandBag then
				if warbandBag:IsOpen() then
					warbandBag:Close()
					print("|cff00ff00AdiBagsEx:|r Warband Bank frame hidden")
				else
					warbandBag:Open()
					print("|cff00ff00AdiBagsEx:|r Warband Bank frame shown")
				end
				
				-- Update button appearance
				self:UpdateWarbandBankButton()
			else
				print("|cffff0000AdiBagsEx:|r Warband Bank module not found")
			end
		end,
		function(self, tooltip)
			local warbandBag = addon:GetModule("WarbandBank")
			if warbandBag and warbandBag:IsOpen() then
				tooltip:AddLine("Hide Warband Bank", 1, 1, 1)
				tooltip:AddLine("Click to hide the Warband Bank frame", 0.8, 0.8, 0.8, true)
			else
				tooltip:AddLine("Show Warband Bank", 1, 1, 1)
				tooltip:AddLine("Click to show the Warband Bank frame", 0.8, 0.8, 0.8, true)
			end
			if not addon.atBank then
				tooltip:AddLine("|cffff4444You must be at a bank to use this feature|r", 1, 0.2, 0.2, true)
			end
		end
	)
	
	-- Store reference to the button for updates
	self.warbandBankButton = button
	self:UpdateWarbandBankButton()
end

function containerProto:UpdateWarbandBankButton()
	if not self.warbandBankButton then
		return
	end
	
	local warbandBag = addon:GetModule("WarbandBank")
	if warbandBag and warbandBag:IsOpen() then
		-- Highlight button when warband bank is visible
		self.warbandBankButton:SetNormalFontObject(GameFontHighlightSmall)
		self.warbandBankButton:SetHighlightFontObject(GameFontHighlightSmall)
	else
		-- Normal appearance when warband bank is hidden
		self.warbandBankButton:SetNormalFontObject(GameFontNormalSmall)
		self.warbandBankButton:SetHighlightFontObject(GameFontHighlightSmall)
	end
end
...
function containerProto:CreateReagentTabButton()
	-- Check if reagent bank is available in this WoW version (removed in 11.2.0)
	local interfaceVersion = select(4, GetBuildInfo())
	if interfaceVersion >= 110200 then
		-- Reagent bank was removed in WoW 11.2.0, don't create the button
		return
	end
	...
	button = self:CreateModuleButton(
		...
		function(_, tooltip)
			if not IsReagentBankAvailable() then
				...
				SetTooltipMoney(tooltip, GetReagentBankCostCompat(), nil, COSTS_LABEL)
				return
			end
			tooltip:AddLine(
				format(
...
function containerProto:UpdateContent(bag)
	...
	-- Safety check: ensure content is initialized for this bag
	if not content then
		self:Debug('UpdateContent: initializing content for bag', bag)
		self.content[bag] = { size = 0 }
		content = self.content[bag]
	end
	...
	for slot = 1, newSize do
		...
		if not itemId or (link and addon.IsValidItemLink(link)) then
			...
			-- Use modern item info APIs for better performance and caching
			local itemData
			if addon.ModernItemInfo then
				itemData = addon.ModernItemInfo:GetContainerItemInfo(bag, slot)
			end
			
			if itemData then
				-- Use modern item data
				name = itemData.name
				...
				subclassID = itemData.subclassID
				bindType = itemData.bindType
				expacID = itemData.expacID
			...
			-- Handle special cases like battlepets
			if link and string.match(link, "|Hbattlepet:") then
				local speciesID = string.match(link, "|Hbattlepet:(%d+):")
				if speciesID then
...
local function FilterByBag(slotData)
	...
	elseif bag == REAGENTBANK_CONTAINER then
		-- Check if Reagent Bank is available in this version
		local interfaceVersion = select(4, GetBuildInfo())
		if interfaceVersion >= 110200 then
			name = nil -- Skip reagent bank in 11.2.0+
		else
			name = REAGENT_BANK
		end
	elseif bag == WARBOUND_BANK_CONTAINER_1 then
		if IsWarboundBankAvailable() then
			name = L["Warbound Bank Tab 1"]
		else
			name = nil -- Skip if not available
		end
	elseif bag == WARBOUND_BANK_CONTAINER_2 then
		if IsWarboundBankAvailable() then
			name = L["Warbound Bank Tab 2"]
		else
			name = nil -- Skip if not available
		end
	elseif bag == WARBOUND_BANK_CONTAINER_3 then
		if IsWarboundBankAvailable() then
			name = L["Warbound Bank Tab 3"]
		else
			name = nil -- Skip if not available
		end
	elseif bag == WARBOUND_BANK_CONTAINER_4 then
		if IsWarboundBankAvailable() then
			name = L["Warbound Bank Tab 4"]
		else
			name = nil -- Skip if not available
		end
	elseif bag == WARBOUND_BANK_CONTAINER_5 then
		if IsWarboundBankAvailable() then
			name = L["Warbound Bank Tab 5"]
		else
			name = nil -- Skip if not available
		end
...
function containerProto:DispatchItem(slotData, fullUpdate)
	...
	if not sectionName then
		-- Prevent error and log warning
		addon:Debug("DispatchItem: sectionName is nil for item:", slotData.link or slotData.name or "none")
		return
	end
