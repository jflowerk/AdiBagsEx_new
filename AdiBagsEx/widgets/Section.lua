...
local sortingFuncs = {
	...
	default = function(linkA, linkB, nameA, nameB)
		-- Use modern item info for better performance
		local qualityA, levelA, classA, subclassA, equipSlotA
		local qualityB, levelB, classB, subclassB, equipSlotB
		
		if addon.ModernItemInfo then
			local _, qA, clsA, subA, dataA = addon.ModernItemInfo:GetItemInfoInstant(linkA)
			local _, qB, clsB, subB, dataB = addon.ModernItemInfo:GetItemInfoInstant(linkB)
			if dataA and dataB then
				qualityA, levelA, classA, subclassA, equipSlotA = qA, dataA.iLevel, dataA.class, dataA.subclass, dataA.equipSlot
				qualityB, levelB, classB, subclassB, equipSlotB = qB, dataB.iLevel, dataB.class, dataB.subclass, dataB.equipSlot
			end
		end
		
		-- Fallback to legacy API if modern data not available
		if not qualityA or not qualityB then
			local _, _, qA, lA, _, cA, sA, _, eA = GetItemInfo(linkA)
			local _, _, qB, lB, _, cB, sB, _, eB = GetItemInfo(linkB)
			qualityA, levelA, classA, subclassA, equipSlotA = qA, lA, cA, sA, eA
			qualityB, levelB, classB, subclassB, equipSlotB = qB, lB, cB, sB, eB
		end
	...
	byQualityAndLevel = function(linkA, linkB, nameA, nameB)
		-- Use modern item info for better performance
		local qualityA, levelA, qualityB, levelB
		
		if addon.ModernItemInfo then
			local _, qA, _, _, dataA = addon.ModernItemInfo:GetItemInfoInstant(linkA)
			local _, qB, _, _, dataB = addon.ModernItemInfo:GetItemInfoInstant(linkB)
			if dataA and dataB then
				qualityA, levelA = qA, dataA.iLevel
				qualityB, levelB = qB, dataB.iLevel
			end
		end
		
		-- Fallback to legacy API if modern data not available
		if not qualityA or not qualityB then
			local _, _, qA, lA = GetItemInfo(linkA)
			local _, _, qB, lB = GetItemInfo(linkB)
			qualityA, levelA = qA, lA
			qualityB, levelB = qB, lB
		end
