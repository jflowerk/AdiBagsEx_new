...
function bankButtonProto:OnClick(button)
	-- Only allow purchase popup if slot is actually purchasable and API exists
	if self.toPurchase and type(GetBankSlotCost) == "function" and type(GetNumBankSlots) == "function" and not addon.isRetail then
...
function bankButtonProto:UpdateStatus()
	local numSlots = type(GetNumBankSlots) == "function" and GetNumBankSlots() or 0
	local bankSlot
	if addon.isRetail then
	...
	if bankSlot <= numSlots then
		...
		local cost = type(GetBankSlotCost) == "function" and GetBankSlotCost(bankSlot) or 0
...
function bankButtonProto:OnShow()
	...
	-- Only register PLAYERBANKBAGSLOTS_CHANGED if it exists in the current client
	if type(self.RegisterEvent) == "function" and (addon.isClassic or addon.isBCC or addon.isWrath or addon.isCata or addon.isMists) then
...
local function Panel_OnShow(self)
	...
	addon:SendMessage('AdiBagsEx_FiltersChanged', true)
end
...
function addon:CreateBagSlotPanel(container, name, bags, isBank)
	...
	LibStub('ABEvent-1.0').RegisterMessage(self:GetName(), 'AdiBagsEx_ConfigChanged', Panel_ConfigChanged, self)
