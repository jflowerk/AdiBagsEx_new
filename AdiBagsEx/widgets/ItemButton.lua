...
function buttonProto:OnReceiveDrag()
	-- Custom drag/drop handler to ensure items go to the correct bank
	if CursorHasItem() then
		-- Check if this is an empty slot
		if not self.hasItem then
			-- Check if this slot belongs to warband bank
			local WARBOUND_CONTAINERS = addon.BAG_IDS.WARBOUNDBANK_ONLY
			if WARBOUND_CONTAINERS and WARBOUND_CONTAINERS[self.bag] then
				-- This is a warband bank slot - place item directly into this container slot
...
local function PreSpawnButtons()
	if addon.OnInitialize then
		...
		-- Fallback: register for addon loaded event
		local frame = CreateFrame("Frame")
		frame:RegisterEvent("ADDON_LOADED")
		frame:SetScript("OnEvent", function(self, event, addonName)
			if addonName == "AdiBagsEx" then
				addon:Debug('Prespawning buttons (fallback)')
				containerButtonPool:PreSpawn(160)
...
function buttonProto:CanUpdate()
	-- Check if bag lag fix experiment is enabled (with safe fallback)
	local useBagLagFix = experiments and experiments.Enabled and experiments:Enabled("Bag Lag Fix")
	
	if useBagLagFix then
		if self.dirty then
			return true
		end
		return false
...
function buttonProto:UpdateBorder(isolatedEvent)
	...
	if self.JunkIcon then
		...
		if addon.ModernItemInfo then
			local _, q = addon.ModernItemInfo:GetItemInfoInstant(self.itemLink or self.itemId)
			quality = q
		end
		if not quality and hasItem(self.hasItem) then
			quality = select(3, GetItemInfo(self.itemLink or self.itemId))
		end
