...
function handlerProto:Set(info, value, ...)
	...
	if self.isFilter then
		self.dbHolder:SendMessage('AdiBagsEx_ConfigChanged', 'filter')
	else
		self.dbHolder:SendMessage('AdiBagsEx_ConfigChanged', path)
	end
...
local function CreateBagOptions(name, key)
	-- Safety check: ensure fonts exist for this key
	if not addon.fonts or not addon.fonts[key] then
		return {
			name = L[name],
			type = 'group',
			args = {
				unavailable = {
					...
					type = 'description',
					order = 1,
					desc = "Theme options for " .. name .. " are not available in this version.",
				}
			}
		}
	end
...
local function GetOptions()
	...
	options = {
		...
		args = {
			enabled = {
				name = L['Enabled'],
				desc = L['Uncheck this to disable AdiBagsEx.'],
				type = 'toggle',
				order = 100,
				disabled = false,
			},
			...
			bags = {
				...
				args = {
					bags = {
						name = L['Enabled bags'],
						desc = L['Select which bags AdiBagsEx should display.'],
						type = 'multiselect',
						order = 90,
						values = bagList,
					},
						...
						args = {
							...
							scale = {
								...
								set = function(info, newScale)
									...
									addon:LayoutBags()
									addon:SendMessage('AdiBagsEx_LayoutChanged')
								end,
							},
							maxHeight = {
					...
					sections = {
						...
						args = {
							compactLayout = {
								...
								desc = L['When enabled, AdiBagsEx reorder the section to achieve a more compact layout.'],
								type = 'toggle',
								order = 135,
							...
							gridLayout = {
								...
								desc = L['When enabled, AdiBagsEx switches to a grid layout with dragable sections.'],
								type = 'toggle',
								order = 135,
								set = function(info, gridLayout)
									addon.db.profile.gridLayout = gridLayout
									ReloadUI()
									addon:SendMessage('AdiBagsEx_GridLayoutChanged')
								end,
							},
							--]]
							columnWidth = {
	...
	-- Reagent bank deposit option - removed in WoW 11.2.0 for all versions
	local interfaceVersion = select(4, GetBuildInfo())
	if interfaceVersion < 110200 then
		options["args"]["bags"]["args"]["automatically"]["args"]["autoDeposit"] = {
			...
			type = 'toggle',
			order = 110,
			disabled = function() return not IsReagentBankAvailable() end,
		}
	end
	...
	-- Add reagent bank theme options for older WoW versions (before 11.2.0)
	local interfaceVersion = select(4, GetBuildInfo())
	if interfaceVersion < 110200 then
		options.args.appearance.args.theme.args.reagentBank = CreateBagOptions("Reagent Bank", "reagentBank")
	end

	LibStub('ABEvent-1.0').RegisterMessage(addonName.."_Config", 'AdiBagsEx_FiltersChanged', UpdateFilterOrder)
